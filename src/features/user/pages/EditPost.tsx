...
export default function EditPost () {
  ...
  const [imageUrl, setImageUrl] = useState<string | null>(null) // Store existing image URL
  ...
  const fetchPostData = async (postId: string) => {
    try {
      const record = await getPostFull(postId)
      ...
      setOldItemName(record.item_name || '')
      // Set all form fields from database
      setAnonymous(record.is_anonymous ? 'yes' : 'no')
      setTitle(record.item_name || '')
      setDesc(record.item_description || '')
      setType(record.item_type === 'lost' ? 'missing' : 'found')
      setCategory(record.category || null)

      // Store existing image URL
      if (record.item_image_url) {
        setImageUrl(record.item_image_url)
      }

      // Parse last seen date and time
      if (record.last_seen_date && record.last_seen_time) {
        // Convert from database format to display format
        const dateObj = new Date(record.last_seen_date)
        ...
        const [hours24, mins] = record.last_seen_time.split(':')
        const hours = parseInt(hours24, 10)
        const mer = hours >= 12 ? 'PM' : 'AM'
        const hours12 = hours % 12 || 12
        setTime(`${hours12}:${mins}`)
        setMeridian(mer as 'AM' | 'PM')
      }

      // Parse location from last_seen_location string
      // Format is typically "Level1 > Level2 > Level3"
      if (record.last_seen_location) {
        const parts = record.last_seen_location.split(' > ').map(p => p.trim())
        setLocationDetails({
          level1: parts[0] || '',
          level2: parts[1] || '',
          level3: parts[2] || ''
      ...
      // Note: Existing image URL will be displayed via imageLink prop
      // User can replace it by uploading a new image
    } catch (error) {
  ...
  } // Fetch post data on mount if postId is provided
  useEffect(() => {
    if (postId) {
  ...
  const handleSubmit = async () => {
    ...
    submitTimeoutRef.current = setTimeout(async () => {
      ...
      console.log(postId ? 'Editing Post:' : 'Submitting New Post:', payload)

      try {
        // Use editPost if postId exists, otherwise createPost
        const result = postId
          ...
          <div className='flex items-center space-x-2'>
            ...
            <div className='text-umak-blue font-default-font text-base font-normal'>
              {postId ? (
                <>
                  Editing Post{' '}
                  <span className='font-semibold'>{oldItemName}</span>
                </>
              ) : (
                'New Post'
              )}
