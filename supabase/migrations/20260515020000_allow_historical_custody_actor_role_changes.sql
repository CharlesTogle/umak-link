...
begin
  ...
  if new.decision_by_guard_id is not null and (
    tg_op = 'INSERT' or (
      tg_op = 'UPDATE' and new.decision_by_guard_id is distinct from old.decision_by_guard_id
    )
  ) then
    select u.user_type
    into v_user_type
  ...
  if new.office_received_by_staff_id is not null and (
    tg_op = 'INSERT' or (
      tg_op = 'UPDATE' and new.office_received_by_staff_id is distinct from old.office_received_by_staff_id
    )
  ) then
    select u.user_type
    into v_user_type
  ...
  if new.investigation_opened_by is not null and (
    tg_op = 'INSERT' or (
      tg_op = 'UPDATE' and new.investigation_opened_by is distinct from old.investigation_opened_by
    )
  ) then
    select u.user_type
    into v_user_type
